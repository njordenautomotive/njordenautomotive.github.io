    ...
    <div aria-hidden className="pointer-events-none fixed inset-0 -z-10 overflow-hidden bg-[#0a0b0d]">
      ...
      <div className="absolute inset-0">
        {Array.from({length:24}).map((_, i) => (
          <div key={i} className="animate-float-slow absolute h-1.5 w-1.5 rounded-full bg-cyan-300/40 shadow-[0_0_12px_rgba(52,211,235,0.35)]"
    ...
    <div className="sticky top-0 z-40 w-full border-b border-white/10 bg-[#0a0b0d]/80 backdrop-blur">
      <div className="mx-auto flex w-full max-w-7xl items-center justify-between px-4 py-3 sm:px-6 lg:px-8">
        <div className="flex items-center gap-3">
          ...
          <span className="font-semibold tracking-wide text-white">NJORDEN</span>
        </div>
        <nav className="hidden items-center gap-6 md:flex">
          ...
          <a href="/" className="text-sm text-white/80 hover:text-white">Journal</a>
        </nav>
        <div className="flex items-center gap-2">
          <a className="rounded-2xl bg-white/90 text-[#0a0b0d] hover:bg-white px-4 py-2 text-sm" href="/newsletter">Join Newsletter</a>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <footer className="border-t border-white/10 py-10">
      <div className="mx-auto flex w-full max-w-7xl flex-col items-center justify-between gap-6 px-4 sm:px-6 lg:px-8 md:flex-row">
        <div className="flex items-center gap-3">
          <div className="h-6 w-6 rounded-xl bg-white/90" />
